# V5 - Multi-AI Research Synthesis Framework

> **V5.** Byte-identical to both [`prompts/Multi-AI_Research_synthesis_prompt.md`](../Multi-AI_Research_synthesis_prompt.md) (the published copy) and `Multi-AI_Research_synthesis_prompt_v5 (by claude).md` in the Dropbox archive - diffed 2026-07-27, no drift in either direction. Duplicated here so the whole set sits side by side; collapse the duplicate once the naming decision is made.
>
> **Standing:** ranked #4 of 6, but scored highest of all versions on discovery breadth (9/10). Two named weaknesses: it treats 3/3 model agreement as high confidence (prominence mistaken for truth), and its section count guarantees a long appendix-heavy report.
>
> **Known blemish, inherited by the published copy:** the REPORT A/B/C placeholders still name files from an old MCP-auth research run instead of reading as generic placeholders. A clean-placeholder variant exists in the archive as `Research_synthesis_prompt_v5 (by claude).md`; that is the only difference between the two.
