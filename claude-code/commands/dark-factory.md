...
5. **Premise-dependent claims with falsifiers.** In the PR body and debrief, label any claim that depends on an unverified premise. State the premise and what would falsify it. Overnight work can't probe back-and-forth, so this gives morning-me a clean review surface.
   ...
   **Calibration: Closes is the DEFAULT once the run has done its job.** When Gate 1 proved the acceptance criteria on real input AND both review layers (the `/ce-...` pass and the Codex peer pass) came back clean with findings applied, write `Closes #N`. "Premise-dependent" means an EMPIRICAL check the run itself cannot execute - a real production scan, a deploy, a quota reset, a third-party callback. It never means "confirm my judgment call": a wording choice, a scope reading, or a design trade that the AIs agreed on is already settled under the standing-proxy contract - record it in the debrief, do not gate the ticket on it. *Why (2026-08-13):* a run left all five merged, dual-review-approved tickets open on confirm-my-call checks; the owner opened the board expecting closed tickets, had to ask what he was being asked to confirm, and closed four of the five by hand. An open ticket whose work is finished is the mirror image of the 2026-07-16 lie: the tracker says "not done" about work that is done.
