...
2. **License MIT at the repo root.** The repo had no root license at all, so outsiders had no general permission to reuse any of it. `python/LICENSE.md` stays proprietary, since it is a template default for new closed projects, and both the LICENSE and the scaffold README now say so explicitly.
...
5. **Make the scaffold's claims true.** Add `pyproject.toml` with ruff and pytest configured, plus a CI workflow running the non-mutating equivalents of the documented commands. Describe the scaffold as minimal, because it is, and say why the source directories are empty on purpose.
...
- **Historic material is now separated from installed material.** Archived work carries a date and a replacement mapping, so it reads as a record rather than as a repo nobody maintains.
- **The root license now grants reuse under MIT,** closing a gap that left outsiders with no permission at all.
